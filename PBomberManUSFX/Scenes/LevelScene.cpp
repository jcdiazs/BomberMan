...
//Una vez creada la clase Enemigo volador, _positionX _type ya no sale error porque es herencia de enemy
void LevelScene::spawnEnemyVolador(GameTexture _texture, AIType _type, const int _positionX, const int _positionY)
{
    auto enemigoVolador = std::make_shared<EnemigoVolador>(gameManager->getAssetManager()->getTexture(GameTexture::Enemy3), gameManager->getRenderer());
    enemigoVolador->setPosition(_positionX, _positionY);
    enemigoVolador->setSize(scaledTileSize, scaledTileSize);
    enemigoVolador->setAIType(_type);

    addObject(enemigoVolador);
    enemiesVolador.push_back(enemigoVolador);
}
...
void LevelScene::update(const unsigned int delta)
{
    ...
    // update timers
    updateTimers(delta);
    //update collision of enemies volador
    updateEnemyVoladorCollision();
}
...
void LevelScene::updateEnemyVoladorCollision()
{
    // iterate enemies for collision
    for (const auto& enemyVolador : enemiesVolador)
    {
        // iterate drawables for collision

        //collisions deberia declarase nuevamente en el .h
        for (const auto& collisionObject : collisionsEnemyVolador)
        {
            // check for block collision
            if (isCollisionDetected(enemyVolador->getRect(), collisionObject.second->getRect()))
            {
                // stop moving on collision detection
                enemyVolador->setMoving(false);
                enemyVolador->revertLastMove();
            }
        }
        // check for bomb collision
        if (bomb && isCollisionDetected(enemyVolador->getRect(), bomb->getRect()))
        {
            // stop moving on collision detection
            enemyVolador->setMoving(false);
            enemyVolador->revertLastMove();
        ...
        if (player != nullptr)
        {
            ...
            playerRect.w = static_cast<int>(playerRect.w * 0.2);
            playerRect.h = static_cast<int>(playerRect.h * 0.2);
            if (isCollisionDetected(playerRect, enemyVolador->getRect()))
            {
                // player killed by enemy
                removeObject(player);
                player = nullptr;
        ...
        {
            // can attack?
            if (!enemyVolador->isMovingToCell() && enemyVolador->canAttack())
            {
                // check for attack radius
                if (abs(player->getPositionX() + player->getWidth() / 2 - enemyVolador->getPositionX() -
                    enemyVolador->getWidth() / 2) < enemyVolador->getAttackRadius() &&
                    abs(player->getPositionY() + player->getHeight() / 2 - enemyVolador->getPositionY() -
                        enemyVolador->getHeight() / 2) < enemyVolador->getAttackRadius())
                {
                    // start follow to player
                    followToPlayer(enemyVolador.get());
                }
            }
        }
    }
...
void LevelScene::updateBangsCollision()
{
    ...
    for(const auto& bang : bangs)
    {
        ...
        auto itEnemiesVolador = enemiesVolador.begin();
        while (itEnemiesVolador != enemiesVolador.end())
        {
            SDL_Rect enemyRect = (*itEnemiesVolador)->getRect();
            enemyRect.w = static_cast<int>(enemyRect.w * 0.2);
            enemyRect.h = static_cast<int>(enemyRect.h * 0.2);
            if (isCollisionDetected(enemyRect, bang->getRect()))
            {
                removeObject(*itEnemiesVolador);
                itEnemiesVolador = enemiesVolador.erase(itEnemiesVolador);
