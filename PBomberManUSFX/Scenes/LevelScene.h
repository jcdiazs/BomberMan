...
private:
    ...
    //Spawn para enemigo que atraviesa todo el mapa
    void spawnEnemyVolador(GameTexture texture, AIType type, const int positionX, const int positionY);
    ...
    //collision para volador
    void updateEnemyVoladorCollision();
    ...
    //enemigo volador, spawn
    std::vector<std::shared_ptr<EnemigoVolador>> enemiesVolador;       // enemiesVolador vec
    std::vector<std::pair<GameTile, std::shared_ptr<GameGraphicObject>>> collisionsEnemyVolador; // collisions
